...
public class Box {
    ...
    public byte[] encrypt(byte[] nonce, byte[] message) {
        ...
        isValid(Sodium.crypto_box_curve25519xsalsa20poly1305(
                ct, msg, msg.length, nonce, publicKey, privateKey), "Encryption failed");
    ...
    public byte[] decrypt(byte[] nonce, byte[] ciphertext) {
        ...
        isValid(Sodium.crypto_box_curve25519xsalsa20poly1305_open(
                message, ct, message.length, nonce, publicKey, privateKey),
